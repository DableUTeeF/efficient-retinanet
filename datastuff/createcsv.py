...
if __name__ == '__main__':
    path = '/media/palm/data/MicroAlgae/jsn/'
    ipath = '/media/palm/data/MicroAlgae/gray_images/'
    csv = []
    classid = []
    for folder in os.listdir(path):
        for sub_folder in os.listdir(os.path.join(path, folder)):
            for file in os.listdir(os.path.join(path, folder, sub_folder)):
                im_json = json.load(open(os.path.join(path, folder, sub_folder, file)))
    ...
    with open('algea/gray_annotations', 'w') as wr:
        for element in csv:
            wr.write(element)
            wr.write('\n')

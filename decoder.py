...
if __name__ == '__main__':
    ...
    pad = 0
    ls = [s.split(',')[0] for s in open('datastuff/algea/val_annotations').read().split('\n')]
    found = []
    # while True:
    # p = os.path.join(path, np.random.choice(os.listdir(path)))
    prd = len(os.listdir('prd'))
    os.mkdir(os.path.join('prd', str(prd)))
    os.mkdir(os.path.join('prd', str(prd), 'correct'))
    os.mkdir(os.path.join('prd', str(prd), 'wrong'))
    os.mkdir(os.path.join('prd', str(prd), 'not_found'))
    os.mkdir(os.path.join('prd', str(prd), 'mixed'))
    for pth in os.listdir(path):
        ...
        img.save(os.path.join('prd', str(prd), flag, pth))
    for l in ls:
        if l not in found:
            print(l)
